...
/**
 * java.io.BufferedReader를 확장하여 readLine() 시 tokenize를 동시에 수행하는
 * readLineToken() 메써드를 추가함.
 * 
 * <br/><br/>중복된 byte 연산을 최소화 하여 파일 입출력 속도 향상을 위함.
 * <br/>readLine() + tokenize() < readLineToken() 
 * 
 * <br/><br/> thread not safe
 * @author Hyoungsoon Kim 
 *
 */

public class AnyframeBufferedReader extends Reader {
	...
	public AnyframeBufferedReader(Reader in, int sz) {
		super(in);
		if (sz <= 0)
	...
	public AnyframeBufferedReader(Reader in) {
	...
	/**
	 * Reader로 부터 한줄을 읽어 {@code this.delimiter}를 구분자로 
	 * tokenized 된 String[] 를 반환 함
	 * 
	...
	public String[] readLineToken(int tokenSize) throws IOException {

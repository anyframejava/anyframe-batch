...
public class DecoderMS949 extends AbstractDecoder {

	private static final int START = 0x41;
	...
	public void decodeChar(byte[] bytes, int byteOffset, int byteLength,
		...
		int byte1, byte2;
		int sPos = byteOffset;
		int ePos = byteOffset + byteLength;
		int charCnt = 0;
		char ch = 0;
		...
		while (sPos < ePos) {
			...
			if (byte1 >= 0) 				// 1byte character
				ch = (char) byte1;
			
			else { 							// 2byte character
				
				if(sPos == ePos) {	// end of input buffer
					sPos++;
					
					if(this.onMalformed == CodingErrorAction.REPORT)
						throw MalformedInputException.newInstance(bytes, byteOffset, byteLength, sPos);
					...
					byte1 &= 0xff;
					byte2 = bytes[sPos++] & 0xff;
	
					// check unmappable character
					if ( (byte1 > index1.length) || ((byte2 < START) || (byte2 > END)) ) {
						
						if(this.onUnmappable == CodingErrorAction.REPLACE)		// replace it
							ch = this.getReplaceChar();
						else if (this.onUnmappable == CodingErrorAction.REPLACE)// throw exception
							throw UnmappableCharacterException.newInstance(bytes, byteOffset, byteLength, sPos);
						else if(this.onUnmappable == CodingErrorAction.IGNORE) 	// ignore character
							continue;
						
					} else {
						
						int n = (index1[byte1] & 0xf) * (END - START + 1) + (byte2 - START);
						ch = index2[index1[byte1] >> 4].charAt(n);

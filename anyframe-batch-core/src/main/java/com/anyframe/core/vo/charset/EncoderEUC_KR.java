...
public class EncoderEUC_KR extends AbstractEncoder {
	...
	public void encodeChar(char[] chars, int charOffset, int charLength, 
		...
		while(curPos < endPos) {
			...
			if (nByte == 1)	// ASCII code
				bytes[byteOffset++] = (byte) c;
			else {
				int offset = index1[((c & 0xff00) >> 8 )] << 8;
				int ncode  = index2[offset >> 12].charAt((offset & 0xfff) + (c & 0xff));
				
				if (ncode == 0) {	// no character matched in this charSet
					if(this.onUnmappable == CodingErrorAction.REPLACE)
						bytes[byteOffset++] = (byte)this.replacement;
					else if(this.onUnmappable == CodingErrorAction.REPORT)
						throw UnmappableCharacterException.newInstance(chars, charOffset, charLength, curPos-charLength);
					...
					bytes[byteOffset++] = (byte) ((ncode & 0xff00) >> 8);
					bytes[byteOffset++] = (byte) (ncode & 0xff);
				}
			}
		}
	...
	/**
	 * Unicode인 {@code chars}를 EUC-KR로 인코딩 했을 때 필요한 byte수를 계산한다.
	 * 

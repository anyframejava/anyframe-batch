...
public class CreateDbDataWithoutVo extends AnyframeAbstractTasklet {
	
	private static final Log LOGGER = LogFactory.getLog(CreateDbDataWithoutVo.class);
	...
	public void execute(AnyframeItemReaderFactory readerFactory,
		...
		// CFG.xml 파일에 정의된 writer의 ID를 기반으로 writerFactory로부터 writer 획득.  
 		...
 		for(int i=0; i<100; i++) {
 			writer.setInt(0, i);
 			writer.setBigDecimal(1, new BigDecimal(i));
 			writer.setBigInteger(2, new BigInteger(String.valueOf(i)));
 			writer.setDate(3, new Date(i));
 			writer.setTimestamp(4, new Timestamp(i));
 			
 			writer.setString(5,"이름_"+i);
 			writer.setString(6,"주소_"+i);
 			writer.setString(7,"설명_"+i);

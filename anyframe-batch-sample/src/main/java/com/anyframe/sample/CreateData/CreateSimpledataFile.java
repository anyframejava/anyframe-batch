...
public class CreateSimpledataFile extends AnyframeAbstractTasklet {
	
	private static final Log LOGGER = LogFactory.getLog(CreateSimpledataFile.class);
	...
	public void execute(AnyframeItemReaderFactory readerFactory,
		...
		// CFG.xml 파일에 정의된 writer의 ID를 기반으로 writerFactory로부터 writer 획득.  
 		...
 		for(int i=0; i<1000; i++) {
 			CreateEmployeeSample.makeData(vo);
			// Employee VO를 파일에 기록
			writer.write(vo);

...
package com.anyframe.sample.Parameters;
...
public class ParameterSample extends AnyframeAbstractTasklet {
	
	private static final Log LOGGER = LogFactory.getLog(ParameterSample.class);

	public void execute(AnyframeItemReaderFactory readerFactory,
                        AnyframeItemWriterFactory writerFactory) throws Exception {

		LOGGER.debug("JOB_PARAM="+ParamUtil.getParameter("TEST"));
		LOGGER.debug("STEP_PARAM="+StepParameterUtil.getStepParam("TEST"));
		
		ParamUtil.addParameter("TEST", "JOBCHANGE");
		StepParameterUtil.addStepParam("TEST", "STEPCHANGE");

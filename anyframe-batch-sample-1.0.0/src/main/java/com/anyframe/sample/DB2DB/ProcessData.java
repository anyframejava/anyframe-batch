...
public class ProcessData extends AnyframeAbstractTasklet {
	...
	public void execute(AnyframeItemReaderFactory readerFactory,
		...
		AnyframeItemReader reader = readerFactory.getItemReader("reader", new EmployeeVo());
		
		writer.loadSQL("insert");
		reader.loadSQL("select");
		
		// reader.loadSQL("select")에 해당하는 파라미터값 , VO세팅
		EmployeeVo condvo = new EmployeeVo();
		condvo.setNo1(20);
		condvo.setName("n_21");
		reader.setSqlParameters(condvo);
		// VO미사용도 가능
		//reader.setSqlParameters(20,"n_21");
		
		EmployeeVo vo;
		// reader로부터 VO를 순차적으로 읽어들임(파일 끝에 도달한 경우 null을 반환함) 
		while((vo = (EmployeeVo) reader.read()) != null ) {
			
			// Employee의 사번(no)이 50보다 큰 경우에만 결과파일에 기록하는 로직
//			if(vo.getNo() >= 50) {
				// 필요한 경우 Logg
				LOGGER.debug("Employee: " + vo);
				writer.write(vo);
				checkCommit();

...
/**
 * java.io.BufferedReader를 확장하여 readLine() 시 tokenize를 동시에 수행하는 readLineToken()
 * 메써드를 추가함.
 * 
 * <br/>
 * <br/>
 * 중복된 byte 연산을 최소화 하여 파일 입출력 속도 향상을 위함. <br/>
 * readLine() + tokenize() < readLineToken()
 * 
 * <br/>
 * <br/>
 * thread not safe
...
public class CSVReader extends Reader {
	...
	private enum CATEGORY {
		EOL, 		// category of end of line char.
		ORDINARY, 	// category of ordinary character
		QUOTE, 		// category of the quote mark
		DELIMITER 	// category of the separator, e.g. comma, semicolon or tab
		//WHITESPACE // category of characters treated as white space.
	}

	private enum STATE {
		START, 			// We are in blanks before the field.
		INPLAIN, 		// We are in the middle of an ordinary field.
		INQUOTED, 		// we are in middle of field surrounded in quotes.
		AFTERENDQUOTE, 	// We have just hit a quote, might be doubled or might be last one.
		EOL
//		SKIPPINGTAIL,	// We are in blanks after the field looking for the separator
	}
	...
	private CATEGORY categorise(char c) {
		switch (c) {
		// case ';':
		// case '!':
		// case '#':
		...
		case '\r':
		case '\n':
			return CATEGORY.EOL; /* artificially applied to end of line */
		case '\"':
	...
	/**
	 * Reader로 부터 한줄을 읽어 {@code this.delimiter}를 구분자로 tokenized 된 String[] 를 반환
	 * 함
	 * 
	 * @return array of string tokens. 파일의 끝이면 null을 반환
	 * 
	 * @see java.io.LineNumberReader#readLine()
	 */

	public String[] readLineTokenEscape() throws IOException {
		lineCount++;
		
		ArrayList<String> tokens = new ArrayList<String>();
		StringBuilder sb = new StringBuilder(CSVReader.defaultExpectedLineLength);
		...
		for (;;) {
			...
			for ( ; nextChar < nChars; nextChar++) {
				c = cb[nextChar];
				CATEGORY category = categorise(c);
				switch (state) {
				case START: {
	...
	public String[] readLineToken() throws IOException {
		ArrayList<String> tokens = new ArrayList<String>();
		StringBuffer s = null;
		int startChar;
